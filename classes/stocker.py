...
# import logging
from classes.miner_updated import mine
...
def stocks_loop(stocks_df):
    ...
    for stock in stocks_grps:
        ...
        dfn.logger.debug(lg)  
        if len(stock_data):
            duration = dfn.invst_dur/2 if "-USD" in stock[0] or "BTC" in stock[0] or "ETH" in stock[0] or "^KMARSP" in stock[0] else dfn.invst_dur
            new_transactions = calc_deltas(stock_data,duration)
            new_transactions["Symbol"] = stock[0]
            transactions = pd.concat([transactions,
                                          new_transactions])
        else:
            Exception(f"____no data for {stock[0]}____")
    transactions = post_calcs(transactions, dfn.invst_dur)
    # transactions["days_deviation"] = dfn.invst_dur-transactions["investment_duration"]
    end = time.perf_counter()
    dfn.logger.info(f"time to run stocks_loop {end - start:.2f} seconds")
    dfn.logger.info(f"tolearnce diff days max: {round(transactions['days_deviation'].max(),0)}")
    return transactions

def post_calcs(dif_table, invst_dur):
    dif_table["investment_duration"] = ((dif_table["withdrawal_date"] -
                               dif_table["investment_date"]).dt.days)/365
    dif_table["days_deviation"] = (dif_table["investment_duration"] - invst_dur)*365
    dif_table["delta"] = dif_table["withdrawal_close"]-dif_table["investment_close"]
    dif_table["profit_percentage"] = 100*(dif_table["withdrawal_close"]/dif_table["investment_close"]-1)
    #delta_in_perc_per_year:
    dif_table["yearly_profit_percentage"] = 100*((dif_table["withdrawal_close"
                           ]/dif_table["investment_close"])**(1/invst_dur)-1)
    return dif_table
